...
export function CancelBookingModal({ booking, onClose, onCancelled }: CancelBookingModalProps) {
  ...
  async function handleConfirm() {
    ...
    setBusy(true);
    clear();
    const { error: err } = await cancelBooking(booking.id, reason);
    setBusy(false);
      ...
      <div className="flex min-h-full items-center justify-center p-6">
        ...
        >
          ...
          <p className="m-0 mb-6 text-[14px] text-muted">
            {booking.services?.name} · {formatDateBR(booking.scheduled_date)} às {formatTimeShort(booking.scheduled_time)}
          </p>
