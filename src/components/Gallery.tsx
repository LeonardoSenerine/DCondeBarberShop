...
export function Gallery({ onOpenLightbox }: GalleryProps) {
  ...
  return (
    <section id="galeria" className="bg-ink px-6 pt-28">
      <div className="mx-auto max-w-[1240px]">
        <div className="mb-12 [animation:dc-up_700ms_ease_both]">
          <span className="font-heading text-xs tracking-[0.36em] text-muted-2 uppercase">Galeria</span>
            ...
            {loopedPhotos.map((g) => (
              ...
              >
                ...
                <span
                  ...
                  style={{ background: "linear-gradient(to top,rgba(10,10,10,0.94) 20%,rgba(10,10,10,0))" }}
                >
                  <span className="font-heading text-[15px] tracking-[0.12em] text-white uppercase">
                    {g.service_label ?? "D'Conde"}
                  </span>
                  <span className="text-[13px] text-silver">{g.client_label}</span>
                </span>
              </button>
            ))}
          </div>
        ...
        <a
          ...
          target="_blank"
          rel="noopener"
          className="mt-2 flex flex-wrap items-center justify-between gap-3.5 rounded-lg border border-border bg-surface px-6 py-5.5 transition-colors hover:border-silver hover:bg-surface-alt"
        >
          <span className="font-heading text-xl font-medium tracking-[0.1em] text-white uppercase">
            Ver mais cortes
          </span>
          <span className="text-sm text-muted">@dcondebarbearia</span>
        </a>
      </div>
    </section>
  );
